...
def entropy_tensor(vals):
    ...
    ent = -1 * torch.sum(freq * torch.log(freq)/torch.log(torch.tensor([2.])).to(freq.device))
    return ent.item()


def compute_block_entropy(map_, poolers):
    with torch.no_grad():
        f = [l(map_.unsqueeze(0).unsqueeze(0).cpu()).reshape(-1) for l in poolers]
...
"""
def most(feats, dims, scales, init_image_size, k_patches=100, dbscan_eps=2, return_mask=False, ks=[1,2,3,4,5]):
    ...
    for i in range(A.shape[0]):
        ...
        # Reshape to 2D (w_featmap, h_featmap)
        ...
        # Compares each entropy with threshold, 
        # if entropy < threshold adds True to pass_ else adds False
        pass_ = [e < t for e, t in zip(ents, thresh)]

        # print('Entropy', ents, 'Threshold', thresh, 'Pass', pass_)

        # If atleast 50% of the maps agree there is an object, we pick it
        if sum(pass_) >= 0.5 * len(pass_):
            seeds.append(i)

    # Sort seeds by entropy
    seeds.sort()

    eee = np.array(eee)
    ttt = np.array(ttt)

    #eee = torch.tensor(eee)
    #ttt = torch.tensor(ttt)

    print("entropy min mean max std")
    print(eee.min(), eee.mean(), eee.max(), eee.std())
    print("threshold min mean max std")
    print(ttt.min(), ttt.mean(), ttt.max(), ttt.std())

    # If there are no seeds, then there are no objects
    if not seeds:
        print('No seeds => No objects')
        return [], A, seeds, []
    ...
    if preds:
        ...
        # If there are multiple boxes, then pick the one with the highest
        # for saliency detection (I'm commenting this out because it's object discovery)
        #else:
